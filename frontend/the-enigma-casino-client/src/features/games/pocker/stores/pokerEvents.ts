...
export const betConfirmedReceived = createEvent<{
  userId: number;
  bet: number;
  totalBet: number;
}>();
export const pokerPhaseChanged = createEvent<
...
export const sendPokerAction = createEvent<{
  move: "fold" | "call" | "check" | "raise" | "all-in";
  amount?: number;
}>();
export const myTurnStarted = createEvent();
export const myTurnEnded = createEvent();
export const blindsAssigned = createEvent<{
  dealer: { userId: number };
  smallBlind: { userId: number; amount: number };
  bigBlind: { userId: number; amount: number };
}>();
...
export const playerActionReceived = createEvent<{
  userId: number;
  move: "raise" | "call" | "fold" | "check" | "all-in";
  amount: number;
  totalBet: number;
}>();
